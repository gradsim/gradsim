...
if __name__ == "__main__":
    ...
    mesh = kal.io.obj.import_mesh(args.infile)
    ...
    # cam_proj = torch.ones(1, 3, 1).to(vertices)
    # cam_transform = torch.ones(1, 4, 3).to(vertices)
    # face_vertices_camera, face_vertices_image, face_normals = \
    #     kal.render.mesh.prepare_vertices(
    #         body_gt.get_world_vertices().unsqueeze(0).repeat(1, 1, 1),
    #         faces[0], cam_proj, camera_transform=cam_transform,
    #     )
    # print(face_vertices_camera.shape, face_vertices_image.shape, textures.shape, face_normals.shape)
    # face_attributes = [
    #     textures.repeat(1, 1, 1, 1),
    #     torch.ones(1, faces.shape[-2], 3, 1).to(textures)
    # ]
    # print(textures.shape, face_attributes[-1].shape)
    # # print(face_attributes.shape)
    # rgb, alpha, face_idx = kal.render.mesh.dibr_rasterization(
    #     256, 256, face_vertices_camera[:, :, :, -1], face_vertices_image, face_attributes, face_normals[:, :, -1]
    # )

    timelapse = kal.visualize.Timelapse(args.logdir)
